...
void main()
{
	...
	float intensityDiffuse = dot(normal, lightdir);
	diff = intensityDiffuse > 0.0 ? intensityDiffuse : 0.0;

	vec3 directionView = normalize(-vPosition);
	vec3 directionHalfway = normalize(lightdir + directionView);
	spec = pow(max(dot(normal, directionHalfway), 0.0), phongExp);
